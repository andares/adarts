...
class DictionaryTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testForBug() {
        $dictionary = new \Adarts\Dictionary();
        $dictionary->add('钱');
        $dictionary->add('测试');
        $dictionary->confirm();
        $keywords = [];
        foreach ($dictionary->seek('钱很多测试') as $result) {
            $keywords[] = $dictionary->getWordByState($result);
        }


        $dictionary->add('走起');
        // 当上面confirm后，再添加再confirm会出现
        // PHP Notice:  Undefined offset: 7
        $dictionary->confirm();
        $keywords = [];
        foreach ($dictionary->seek('钱很多测试走起拉') as $result) {
            $keywords[] = $dictionary->getWordByState($result);
        }
        /*
        $this->object
